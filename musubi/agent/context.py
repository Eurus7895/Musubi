...
_VERBOSITY_NOTE = (
    ...
    "directly. Any worker flow starts with `musubi_begin_plan`: use bounded "
    "Read/Grep/Glob calls yourself, then call `musubi_commit_plan` with plan "
    "markdown, a compact manifest, your own small/medium/large decision, and "
    "the ordered worker chain. A one-file change is a plan with one file in "
    "its manifest and one role in its chain — small work is proved small by "
    "the manifest, never asserted before reading. Do not spawn a planner; you "
    "are the planner. Give workers compact implementation-ready briefs; let "
    "the coder choose append chunks or split files and require UTF-8. "
