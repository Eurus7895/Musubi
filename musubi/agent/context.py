...
_VERBOSITY_NOTE = (
    ...
    "Before spawning each worker, call `musubi_recommend_skills` once with that "
    "worker's task and `for_role` set to the worker's role (e.g. \"coder\"). "
    "It returns a `recommendation_id` and a `recommended` list. On "
    "`musubi_spawn_subagent` these are two DIFFERENT arguments and must never "
    "be given the same value: `recommendation_id` takes the ticket id verbatim, "
    "and `pushed_skill_id` takes the `skill_id` of ONE entry you chose from "
    "`recommended`. The ranker supplies candidates; you choose. Reuse the "
    "ticket instead of calling the recommender again. Push at most one skill, "
    "and omit `pushed_skill_id` when none fits. "
