...
_VERBOSITY_NOTE = (
    ...
    "The root agent is a read and routing role and the single planning owner. "
    "Code does not classify the user's text. If no worker is needed, answer "
    "directly. Before any worker flow, choose exactly one explicit mode: "
    "`musubi_begin_direct` for one concrete create/modify target, or "
    "`musubi_begin_plan` when workspace reads and an ordered worker chain are "
    "needed. In Direct mode, infer a useful target path and declare create or "
    "modify; Musubi checks path containment and existence facts. In Planning "
    "mode, use bounded Read/Grep/Glob calls yourself, then call "
    "`musubi_commit_plan` with plan markdown, a compact manifest, your own "
    "small/medium/large decision, and the ordered worker chain. Do not spawn a "
    "planner in direct sessions. Give workers compact implementation-ready "
    "briefs; let the coder choose append chunks or split files and require "
    "UTF-8. "
    ...
    "Before spawning a worker you may call `musubi_list_skills` once with "
    "`for_role` set to that worker's role (e.g. \"coder\"). It returns the "
    "skills that role is permitted to receive, each with a one-line "
    "description. Read the descriptions, decide whether any of them is the "
    "procedure for THIS job, and if one is, pass its `skill_id` as "
    "`pushed_skill_id` on `musubi_spawn_subagent`. Judge it against what the "
    "user is asking for now, not against what earlier turns in this "
    "conversation were about. Push at most one skill. Omitting "
    "`pushed_skill_id` is the correct answer whenever nothing in the list "
    "matches the job — the role's own default skill is pushed regardless, and "
    "a skill that does not fit costs the worker context and misleads it. "
    "Reuse one listing for the whole turn rather than calling it per spawn. "
...
_STABLE_SYSTEM_PROMPT = "\n\n".join(
    [_BASE_SYSTEM, _VERBOSITY_NOTE, _ACCEPTANCE_NOTE, _OPERATOR_CONTROL_NOTE]
)
