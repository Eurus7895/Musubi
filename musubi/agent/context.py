...
_VERBOSITY_NOTE = (
    ...
    "Before spawning a worker call `musubi_list_skills` once with "
    "`for_role` set to that worker's role (e.g. \"coder\"). It returns the "
    "skills that role is permitted to receive, each with a one-line "
    "description. Read the descriptions, choose the procedure for THIS job, "
    "and pass its `skill_id` as "
    ...
    "conversation were about. Push exactly one skill. The harness validates "
    "and injects your choice; it never chooses, defaults, substitutes, or "
    "silently drops a skill. If no listed skill fits, do not spawn that role. "
