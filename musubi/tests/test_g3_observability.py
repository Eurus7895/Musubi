...
def test_musubi_record_agent_cycle_writes_a_row(fresh_db: Path) -> None:
    sid = state.create_session("do x", fresh_db)
...
def test_musubi_query_agent_cycles_returns_rows(fresh_db: Path) -> None:
    sid = state.create_session("do x", fresh_db)
    server.musubi_record_agent_cycle(
        session_id=sid, stage="plan", attempt=1, cycle_idx=0,
        started_at=1.0, ended_at=2.0, lm_ms=100,
    )
    raw = server.musubi_query_agent_cycles(session_id=sid)
...
def test_musubi_record_stage_metric_writes_a_row(fresh_db: Path) -> None:
    sid = state.create_session("do x", fresh_db)
    raw = server.musubi_record_stage_metric(
        session_id=sid, stage="plan", attempt=1,
        started_at=1.0, ended_at=2.0,
        tokens_in_estimate=500, tokens_out_estimate=200,
        lm_ms=1000,
    ...
    assert json.loads(raw)["status"] == "ok"
    rows = json.loads(server.musubi_query_stage_metrics(sid))["rows"]
    assert len(rows) == 1


def test_musubi_finalize_rejects_unknown_status(fresh_db: Path) -> None:
    sid = state.create_session("do x", fresh_db)
    raw = server.musubi_finalize_pipeline_run(
        session_id=sid, final_status="exploded",
    )
    assert json.loads(raw)["status"] == "error"


def test_musubi_finalize_derives_tokens_and_corrections(fresh_db: Path) -> None:
    sid = state.create_session("do x", fresh_db)
    state.increment_attempt(sid, "code", fresh_db)  # one correction
    server.musubi_record_stage_metric(
...
def test_musubi_query_pipeline_runs_filters_by_pipeline_name(
    ...
    server.musubi_finalize_pipeline_run(sid_a, "success")
    server.musubi_finalize_pipeline_run(sid_b, "success")
    raw = server.musubi_query_pipeline_runs(pipeline_name="code-review")
...
def test_aggregate_pipeline_stats_5_runs(fresh_db: Path) -> None:
    ...
    for sid, (status, esc, tok) in zip(sids, finals, strict=True):
        ...
        server.musubi_finalize_pipeline_run(
            sid, status, escalated=esc, chunked=False, chunk_count=0,
        )

    raw = server.musubi_pipeline_stats(pipeline_name="feature-dev")
...
def test_pipeline_stats_excludes_in_flight_runs(fresh_db: Path) -> None:
    ...
    server.musubi_finalize_pipeline_run(sid_done, "success")
...
# ── Integration: musubi_new_session opens the row ────────────────────


def test_musubi_new_session_opens_pipeline_runs_row(fresh_db: Path) -> None:
    raw = server.musubi_new_session("test request", pipeline_name="feature-dev")
