...
Mirrors the sequence the dispatcher fires for a chunked coder run:
  1. coder pre-spawn → musubi_spawn_subagent(role='explorer',
     parent_agent_name='coder')
  2. extension-side runner completes → musubi_complete_subagent(
     status='done', summary, tools_used, turns)
  3. parent reads back → musubi_query_subagent_events(
...
@pytest.fixture()
def musubi_db(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
...
def test_coder_explorer_spawn_writes_audit_row(
    ...
    sid = state.create_session("build a feature", musubi_db)
