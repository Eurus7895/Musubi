...
# ── musubi_get_skill — allowlist enforcement ─────────────────────────────────

def test_unauthorized_agent_cannot_load_skill() -> None:
    result = json.loads(server.musubi_get_skill("python", "planner"))
    assert "error" in result
    assert "not permitted" in result["error"].lower()


def test_unauthorized_skill_for_agent_blocked() -> None:
    result = json.loads(server.musubi_get_skill("devops", "coder"))
    assert "error" in result
    assert "not permitted" in result["error"].lower()
...
def test_authorized_skill_not_blocked_by_allowlist() -> None:
    ...
    result_str = server.musubi_get_skill("python", "coder")
...
def test_reviewer_code_review_not_blocked() -> None:
    result_str = server.musubi_get_skill("code-review", "reviewer")
    if result_str.startswith("{"):
        result = json.loads(result_str)
        assert "not permitted" not in result.get("error", "").lower()
...
def test_unauthorized_reference_blocked() -> None:
    ...
    result = json.loads(server.musubi_get_reference("code-review", "owasp.md", "coder"))
    assert "error" in result
    assert "not permitted" in result["error"].lower()
...
def test_authorized_reference_passes_allowlist() -> None:
    ...
    result = json.loads(server.musubi_get_reference("python", "nonexistent.md", "coder"))
    assert "not permitted" not in result.get("error", "").lower()
...
def test_required_skill_in_allowlist_is_injected(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = json.loads(server.musubi_read_stage("sess1", "design", "coder"))
...
def test_required_skill_not_in_allowlist_is_blocked(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = json.loads(server.musubi_read_stage("sess2", "design", "coder"))
...
def test_reviewer_no_memory_injection(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = json.loads(server.musubi_read_stage("sess-rv1", "code", "reviewer"))
...
def test_coder_still_gets_memory_injection(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = json.loads(server.musubi_read_stage("sess-cd1", "design", "coder"))
...
# ── musubi_list_skills filtering ────────────────────────────────────────────


def test_musubi_list_skills_filters_to_caller_allowlist() -> None:
    """Catalog returned to a caller must contain only skills it can load."""
    raw = server.musubi_list_skills("coder")
...
def test_musubi_list_skills_reviewer_contains_code_review_only() -> None:
    ...
    payload = json.loads(server.musubi_list_skills("reviewer"))
    ids = {s["skill_id"] for s in payload["skills"]}
