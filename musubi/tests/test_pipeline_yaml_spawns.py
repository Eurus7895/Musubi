...
# ── musubi_list_subagent_spawns MCP tool ────────────────────────────────────

def test_list_subagent_spawns_tool_returns_yaml_intersection() -> None:
    raw = server.musubi_list_subagent_spawns("feature-dev", "coder")
    out = json.loads(raw)
...
def test_list_subagent_spawns_tool_for_unknown_pipeline() -> None:
    out = json.loads(server.musubi_list_subagent_spawns("nope", "coder"))
    assert out["status"] == "ok"
    assert out["roles"] == []


def test_list_subagents_accepts_pipeline_name() -> None:
    """musubi_list_subagents was extended with an optional pipeline_name
    arg; passing it should narrow the result through pipeline.yaml."""
    raw = server.musubi_list_subagents("coder", pipeline_name="feature-dev")
    out = json.loads(raw)
...
def test_list_subagents_back_compat_without_pipeline() -> None:
    ...
    raw = server.musubi_list_subagents("coder")
    out = json.loads(raw)
    role_names = {r["role"] for r in out["roles"]}
    assert role_names == {"explorer", "investigator"}
