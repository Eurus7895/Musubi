...
Two layers:
  ...
  2. Integration tests on `musubi_complete_subagent` — wires the
...
# ── Integration: musubi_complete_subagent wiring ───────────────────────────
...
# ── musubi_get_subagent_context (Phase A.2 firewall MCP tool) ──────────────

def test_get_context_returns_only_firewalled_fields(mcp_db: Path) -> None:
    parent = state.create_session("p")
    h = _spawn_explorer(parent)
    raw = server.musubi_get_subagent_context(handle_id=h)
    payload = json.loads(raw)
...
def test_get_context_unknown_handle_errors(mcp_db: Path) -> None:
    raw = server.musubi_get_subagent_context(handle_id="nope")
    payload = json.loads(raw)
