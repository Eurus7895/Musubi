...
Covered:
  ...
  - server.musubi_append_failure_pattern: MCP envelope, dedup signal,
    bad-input rejection.
  - server.musubi_delete_subsessions_for_parent: prunes terminal rows,
...
# ── server.musubi_append_failure_pattern ────────────────────────────────────
...
def test_mcp_append_failure_pattern_round_trip(
    ...
    import server
    raw = server.musubi_append_failure_pattern(
    ...
    raw2 = server.musubi_append_failure_pattern(
        "coder", "schema drift between planner and coder", "reviewer-fail",
    )
...
def test_mcp_append_failure_pattern_rejects_empty(
    ...
    monkeypatch.setattr(session_distiller, "_DEFAULT_REPO_ROOT", repo_root)
    import server
    raw = server.musubi_append_failure_pattern("", "issue")
    payload = json.loads(raw)
    assert payload["status"] == "error"
    raw = server.musubi_append_failure_pattern("agent", "")
    payload = json.loads(raw)
    assert payload["status"] == "error"


# ── server.musubi_delete_subsessions_for_parent ────────────────────────────
...
def test_delete_subsessions_prunes_terminal_only(state_db: Path) -> None:
    ...
    import server
    raw = server.musubi_delete_subsessions_for_parent(parent)
    payload = json.loads(raw)
...
def test_delete_subsessions_age_gates(state_db: Path) -> None:
    ...
    import server
    raw = server.musubi_delete_subsessions_for_parent(parent, past_cutoff)
    ...
    raw = server.musubi_delete_subsessions_for_parent(parent, future_cutoff)
