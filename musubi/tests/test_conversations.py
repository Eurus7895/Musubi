"""Tests for session/conversations.py and the musubi_append_message /
musubi_get_conversation MCP tools (Phase C.1).
...
def test_mcp_append_then_get_round_trip(
    ...
    appended = json.loads(
        server.musubi_append_message("chat-X", "user", "ping")
    )
    assert appended["status"] == "ok"
    assert appended["tokens_estimate"] >= 1

    fetched = json.loads(server.musubi_get_conversation("chat-X"))
...
def test_mcp_append_rejects_bad_role(
    ...
    out = json.loads(server.musubi_append_message("chat-X", "wizard", "hi"))
...
def test_mcp_append_coerces_dict_content_to_json_string(
    ...
    appended = json.loads(server.musubi_append_message(
        "chat-X", "tool",
        {"tool": "musubi_spawn_subagent", "result": "{\"ok\": true}"},
    ))
    assert appended["status"] == "ok"
...
def test_mcp_append_rejects_unserializable_content(
    ...
    out = json.loads(server.musubi_append_message("chat-X", "user", {1, 2, 3}))
    assert out["status"] == "error"
