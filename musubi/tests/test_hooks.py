...
# ── musubi_run_hook MCP tool ────────────────────────────────────────────────

def test_musubi_run_hook_reports_no_config_for_unknown_event(monkeypatch) -> None:
    import server

    out = json.loads(server.musubi_run_hook("NonexistentEvent", ""))
    assert out["event"] == "NonexistentEvent"
    assert out["results"] == []


def test_musubi_run_hook_runs_pre_tool_use(monkeypatch) -> None:
    import server
    ...
    out = json.loads(server.musubi_run_hook("PreToolUse", payload))
...
def test_musubi_run_hook_deny_surfaces_exit_1(monkeypatch) -> None:
    import server
