...
Tier 1 — MEMORY.md (~200 tokens, always injected by musubi_read_stage)
...
Tier 2 — .github/memory/*.md (loaded on demand via get_tier2_entry)
    ...
    agents request specific entries via musubi_get_reference("memory", name).
...
# memory_loader.py lives in musubi/memory/ → repo root is two levels up
...
def get_memory_context(repo_root: Path | None = None) -> dict:
    """Return a dict with Tier 1 index ready for injection into musubi_read_stage.
