...
def _resolve_skills_dir() -> Path:
    # When running as a bundled extension binary, MUSUBI_ROOT is set by the
    # VS Code extension to its own extensionPath (where .github/ is shipped).
    musubi_root = os.environ.get("MUSUBI_ROOT")
    if musubi_root:
        return Path(musubi_root) / ".github" / "skills"
    # Development: resolve relative to this file's repo root.
    # __file__ = musubi/skills/skill_loader.py → repo root is 3 up.
