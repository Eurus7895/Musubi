...
Two public APIs:
    ...
    read_stage_for_agent(session_id, stage, agent_name) → dict | None
        Used by musubi_read_stage MCP tool — per-stage access with
...
# ── Per-stage MCP access (used by musubi_read_stage tool) ───────────────────

# Which stages each agent is permitted to read via musubi_read_stage.
