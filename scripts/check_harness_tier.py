#!/usr/bin/env python3
"""Fail CI when new/modified substrate files lack a `harness-tier:` tag.

harness-tier: substrate
expires-when: never — this is the self-enforcement mechanism for HI #9.

Usage:
    python scripts/check_harness_tier.py            # check ALL files in scope
    python scripts/check_harness_tier.py --diff     # check only files modified vs origin/dev

Scope (files MUST declare `harness-tier:`):
  - copilot-harness/**/*.py (excluding tests/, __pycache__)
...
TIER_RE = re.compile(r"harness-tier:\s*(substrate|ephemeral)", re.IGNORECASE)
...
def check_file(path: Path) -> list[str]:
    ...
    if not tier_match:
        return [f"{path}: missing `harness-tier:` declaration"]
...
def main() -> int:
    ...
    if not files:
        print("[check-harness-tier] no in-scope files to check")
        return 0
    ...
    if all_failures:
        ...
        print(f"\n[check-harness-tier] {len(all_failures)} violation(s) "
              f"across {len(files)} file(s)", file=sys.stderr)
        return 1

    print(f"[check-harness-tier] OK — {len(files)} files all carry tags")
    return 0
