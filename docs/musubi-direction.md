...
1. **What is the goal of Musubi?** — sharpens CLAUDE.md's
...
The current one-liner — _"Copilot Chat reasons. Musubi controls
the environment."_ — is correct but soft. Sharpened:

> **Musubi is a governance layer for agentic software-
...
|---|---|---|
| `storage/db.py` tables (`stage_outputs`, `stage_metrics`, `subagent_audit`, `sessions`, `pipeline_runs`, `agent_turns`, `conversation_messages`) | Evaluation data outlives every model release | Add `agent_cycles` (L2) so per-cycle data is queryable |
| `.github/skills/<name>/SKILL.md` catalog | Article's "fat skills" — domain expertise in the cheapest-to-edit surface | Distill more skills from real sessions via `musubi_distill_session` |
...
| MCP tool catalog (`musubi_*`) | The interface between harness and any agent runtime | Stable; new tools added on need only |
...
- `musubi-tier: substrate` — invest, refactor, build on
- `musubi-tier: ephemeral · expires-when: <model capability> · cost-lever: <credits saved>` — track, don't harden, ready to delete
...
## 3. Convergence path — agent as the universal governed surface
...
| Agent mode (`@harness <prompt>`) | Free-form chat; any task | Sub-agent firewall, audit (`conversation_messages`), partial — no validator, no correction loop, no budget enforcer registered |
...
The agent already has the right shape — agent + tools + skills,
...
The model picks the skill (already true in agent mode via
`musubi_get_skill` + `musubi_list_skills`). The **harness picks
...
| Failure patterns (memory tier-2, existing) | "Tried skill X here, failed because Y" | Grows as `musubi_distill_session` fires |
...
This makes the agent's pull model **context-aware** without
...
gets Musubi to Browser-Use-scale (~600 lines of harness).
...
**A.3 — Distill skills from real sessions.** Push `musubi_distill_
...
**C.1 — Add the `musubi-tier` tag to every component.** One PR that
...
~600 lines of harness. Musubi is ~10k+ TS + Python.
...
Musubi already has the right primitives:
...
> _"Every PR moves Musubi either toward thicker substrate
...
`musubi-tier: ephemeral · expires-when: one-model context-cached
