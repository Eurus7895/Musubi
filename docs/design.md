...
| Routing | Zero-cost routing (`/<pipeline>` → pipeline; anything else → orchestrator) | **substrate** |
...
-- One row per chat message (orchestrator + sub-agent results)
...
| Tool | Purpose |
|---|---|
| `harness_get_skill` | Load `SKILL.md` on demand (orchestrator path) |
...
| `harness_record_orchestrator_turn` | Per orchestrator turn row |
| `harness_query_orchestrator_turns` | All orchestrator_turns for a chat_id |
...
- Routing is regex-based (`/<pipeline-name>` matches a slash command,
  anything else goes to orchestrator) — no LLM call to decide
