...
# Concrete VS Code LM tool names this agent advertises to the model.
# Aggressively curated to 3 conceptual tools — every entry's
# description + JSON schema costs ~200-300 tokens of catalog metadata
# re-sent on every sendRequest cycle, so each tool needs to earn its
# slot. Copilot ships two naming conventions; both forms are listed
# so we resolve correctly regardless of which the install registers.
#
# Kept (3 tools, one per common verb):
#   search content    — covers "find references", "where is", "grep"
#   read a known file — the universal read
#   replace a string  — covers most edit cases
#
# Dropped (and why):
#   listDirectory / findFiles → covered by searchWorkspace
#   semanticSearch / searchSymbols → covered by searchWorkspace
#   getErrors → rare; integrated terminal handles diagnostics
#   fetchWebPage → web fetch on a code task is usually wrong
#   getChangedFiles → terminal `git status` is faster
#   insertEdit → replaceString covers most edits
#   create_file → wholesale create crosses into pipeline territory
#
# When destructive / shell / wholesale-create intent is needed, the
# routing skill's "destructive operation" section directs the user to
# `/feature-dev` or the integrated terminal — not by adding tools here.
lm_tools:
