...
harness-tier: substrate test — pins the path-safety boundary and the
...
def test_mcp_read_file_returns_json(workspace: Path) -> None:
    ...
    payload = json.loads(server.harness_read_file("f"))
    assert payload["status"] == "ok"
    assert payload["content"] == "xyz"


def test_mcp_write_file_round_trip(workspace: Path) -> None:
    payload = json.loads(server.harness_write_file("out.txt", "hello"))
    assert payload["status"] == "ok"
...
def test_mcp_edit_file_round_trip(workspace: Path) -> None:
    ...
    payload = json.loads(server.harness_edit_file("f", "abc", "XYZ"))
    assert payload["status"] == "ok"
...
def test_mcp_run_command_round_trip(workspace: Path) -> None:
    payload = json.loads(server.harness_run_command("echo ok"))
    assert payload["status"] == "ok"
