...
def test_harness_record_agent_turn_inserts_row(fresh_db: Path) -> None:
    raw = server.harness_record_agent_turn(
        chat_id="chat-mcp",
...
def test_harness_query_agent_turns_returns_rows(fresh_db: Path) -> None:
    server.harness_record_agent_turn(
    ...
    raw = server.harness_query_agent_turns("chat-q")
    result = json.loads(raw)
    assert result["status"] == "ok"
...
def test_harness_query_agent_turns_empty_chat(fresh_db: Path) -> None:
    raw = server.harness_query_agent_turns("never-existed")
    result = json.loads(raw)
    assert result["status"] == "ok"
    assert result["rows"] == []
