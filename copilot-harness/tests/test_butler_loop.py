...
def test_loop_dispatches_real_tool_and_feeds_result_back(
    ...
    """Two-cycle interaction:
      cycle 0: model asks for harness_new_session
    ...
    assert "session_id" in blocks[0]["content"], "harness_new_session must return a session_id"
...
def test_loop_aborts_after_max_cycles(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    looping_response = LMResponse(
        ...
        content=[{
            "type": "tool_use",
            "id": "x",
            "name": "harness_get_active_session",
            "input": {},
        }],
    )
