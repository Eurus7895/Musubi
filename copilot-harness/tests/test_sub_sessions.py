...
# isolated tmp file per test so the harness_* functions read/write there.
...
def mcp_db(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    # Fast polling so harness_await_subagent doesn't drag the suite.
...
def test_mcp_spawn_then_complete_then_await_returns_summary(
    ...
    await_raw = server.harness_await_subagent(handle_id=h, max_wait_s=2)
...
def test_mcp_await_unknown_handle_errors(mcp_db: Path) -> None:
    raw = server.harness_await_subagent(handle_id="nope", max_wait_s=1)
    payload = json.loads(raw)
    assert payload["status"] == "error"
...
def test_mcp_await_pending_returns_snapshot_after_max_wait(
    ...
    raw = server.harness_await_subagent(handle_id=spawn["handle_id"], max_wait_s=0)
    payload = json.loads(raw)
...
def test_mcp_await_wall_clock_kill_escalates(
    ...
    time.sleep(1.1)
    raw = server.harness_await_subagent(handle_id=spawn["handle_id"], max_wait_s=2)
    payload = json.loads(raw)
    assert payload["status"] == "recorded"
...
def test_mcp_list_subagents_for_unknown_main_is_empty(mcp_db: Path) -> None:
    raw = server.harness_list_subagents(main_agent_name="nobody")
    payload = json.loads(raw)
    assert payload["roles"] == []
