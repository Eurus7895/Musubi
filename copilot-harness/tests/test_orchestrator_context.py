"""Tests for the Orchestrator slice of the harness (Phase B.1).

Covers:
  - validation/context_builder.py: dispatcher accepts "orchestrator",
    returns memory_tier1 only, exposes no pipeline-stage state, denies
    every harness_read_stage call via _STAGE_PERMISSIONS.
  - validation/context_builder.AGENT_SKILL_ALLOWLIST gates the
    orchestrator-routing skill but nothing else.
  - scripts/policy_engine.py: orchestrator's spawn_allowlist includes
  ...
  - .github/agents/orchestrator.agent.md frontmatter declares the
    expected sees/spawn/budget contract.
  - .github/skills/orchestrator-routing/SKILL.md exists and has the
...
# ── _context_orchestrator: shape + firewall ─────────────────────────────────

def test_orchestrator_dispatcher_accepts_orchestrator(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """build_context must route 'orchestrator' to the new builder."""
    monkeypatch.setattr(memory_loader, "get_memory_context", lambda: {})
    ctx = context_builder.build_context("ignored-session-id", "orchestrator")
    assert isinstance(ctx, dict)


def test_orchestrator_context_returns_only_memory_tier1(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        memory_loader,
    ...
    ctx = context_builder.build_context("ignored", "orchestrator")
    assert set(ctx.keys()) == {"memory_tier1"}
    assert ctx["memory_tier1"]["tier1_index"].startswith("# Tier 1")


def test_orchestrator_context_empty_when_no_memory(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """get_memory_context returns {} when MEMORY.md is absent — the
    orchestrator must still get a dict with the key, just empty."""
    monkeypatch.setattr(memory_loader, "get_memory_context", lambda: {})
    ctx = context_builder.build_context("ignored", "orchestrator")
    assert ctx == {"memory_tier1": {}}


def test_orchestrator_context_has_no_pipeline_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The orchestrator must never see request, plan, design, code, review,
...
def test_orchestrator_context_has_no_runner_supplied_keys(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_orchestrator_session_id_is_ignored(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The orchestrator does not have a pipeline session — passing any
    value (real, fake, or unused) must produce the same context."""
    monkeypatch.setattr(memory_loader, "get_memory_context", lambda: {"a": 1})
    ctx_a = context_builder.build_context("session-a", "orchestrator")
    ctx_b = context_builder.build_context("session-b", "orchestrator")
    ctx_c = context_builder.build_context("", "orchestrator")
    assert ctx_a == ctx_b == ctx_c


def test_unknown_agent_message_now_lists_orchestrator() -> None:
    """The dispatcher's error message must mention orchestrator so a
    misspelled agent name surfaces it as a valid option."""
    with pytest.raises(ValueError, match="orchestrator"):
        context_builder.build_context("sid", "orcestrator")


# ── _STAGE_PERMISSIONS: orchestrator denies every stage read ────────────────

@pytest.mark.parametrize("stage", ["plan", "design", "code", "review"])
def test_orchestrator_cannot_read_any_pipeline_stage(stage: str) -> None:
    """harness_read_stage must return None for the orchestrator on any
    pipeline stage — pipelines are user-invoked and run in their own
    session; the orchestrator must not peek."""
    result = context_builder.read_stage_for_agent(
        "any-session", stage, "orchestrator"
    )
    assert result is None


# ── AGENT_SKILL_ALLOWLIST: orchestrator-routing only ────────────────────────

def test_orchestrator_skill_allowlist_includes_routing() -> None:
    """The runner pushes orchestrator-routing via inject_skills frontmatter
    and may also call harness_get_skill('orchestrator-routing') on demand;
    the policy gate must accept that call."""
    assert "orchestrator-routing" in AGENT_SKILL_ALLOWLIST["orchestrator"]


def test_orchestrator_skill_allowlist_excludes_generator_skills() -> None:
    """The orchestrator does not author code; loading python / api-design /
    code-review etc. into its context would blur the dispatcher boundary."""
    forbidden = {
        "python", "api-design", "testing", "database-patterns",
        "documentation", "code-review",
    }
    assert AGENT_SKILL_ALLOWLIST["orchestrator"].isdisjoint(forbidden)


def test_orchestrator_skill_permission_check() -> None:
...
def test_orchestrator_spawn_allowlist_includes_phase_a_roles() -> None:
    roles = set(MAIN_SUBAGENT_ALLOWLIST["orchestrator"])
    assert {"explorer", "investigator", "reviewer-aux"}.issubset(roles)


def test_orchestrator_spawn_allowlist_includes_pipeline_roles() -> None:
    """Locked decision #4 — orchestrator may spawn individual pipeline
    roles ad-hoc."""
    roles = set(MAIN_SUBAGENT_ALLOWLIST["orchestrator"])
    assert {"planner", "coder", "reviewer"}.issubset(roles)


def test_orchestrator_cannot_spawn_designer() -> None:
    """Designer is a pipeline-internal stage. Spawning it ad-hoc is
    not in scope for B.1; if the orchestrator needs design work it
    should ask the user for /feature-dev."""
    assert check_subagent_allowed("orchestrator", "designer") is False


def test_orchestrator_cannot_spawn_unknown_role() -> None:
    assert check_subagent_allowed("orchestrator", "saboteur") is False


def test_orchestrator_can_spawn_each_listed_role() -> None:
    for role in MAIN_SUBAGENT_ALLOWLIST["orchestrator"]:
        assert check_subagent_allowed("orchestrator", role) is True, role


def test_orchestrator_list_includes_six_roles() -> None:
...
def test_orchestrator_deny_reason_for_unknown_role_lists_pipeline_roles() -> None:
...
def test_pipeline_roles_have_tool_policies() -> None:
    ...
    so harness_spawn_subagent doesn't fail-closed on an unknown role."""
...
def test_coder_subagent_can_write_when_main_grants_write() -> None:
    """An orchestrator-spawned coder needs Write/Edit/Bash to be useful.
    The orchestrator's own tool list does NOT include those, so in
    practice the intersection drops them — verify the math here."""
    main_tools = ["Read", "View", "Grep", "Glob"]  # orchestrator's tools
    eff = effective_subagent_tools("orchestrator", main_tools, "coder")
...
_AGENT_FILE = _REPO_ROOT / ".github" / "agents" / "orchestrator.agent.md"
_SKILL_FILE = (
    _REPO_ROOT / ".github" / "skills" / "orchestrator-routing" / "SKILL.md"
)
...
def test_orchestrator_agent_frontmatter_declares_contract() -> None:
...
def test_orchestrator_agent_declares_three_per_role_spawn_cap() -> None:
    text = _AGENT_FILE.read_text(encoding="utf-8")
    assert "max_spawns_per_role_per_turn: 3" in text


def test_orchestrator_agent_lists_six_spawn_roles() -> None:
    text = _AGENT_FILE.read_text(encoding="utf-8")
    for role in (
        "explorer", "investigator", "reviewer-aux",
        "planner", "coder", "reviewer",
...
def test_orchestrator_agent_disallows_writes() -> None:
    """The orchestrator routes; it must not write to disk itself."""
    text = _AGENT_FILE.read_text(encoding="utf-8")
    assert 'disallowedTools: ["Write", "Edit", "Bash"]' in text


def test_orchestrator_routing_skill_file_exists() -> None:
    assert _SKILL_FILE.is_file()


def test_orchestrator_routing_skill_has_name_frontmatter() -> None:
    text = _SKILL_FILE.read_text(encoding="utf-8")
    assert text.startswith("---")
    assert "name: orchestrator-routing" in text
...
def test_planner_context_unchanged_by_orchestrator_addition(
    ...
    """Adding the orchestrator branch must not bleed memory or other keys
    into the existing planner path."""
    from session import state
    from storage import db as _db

    tmp_root = Path(_REPO_ROOT) / "copilot-harness" / "tests"
