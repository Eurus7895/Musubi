...
`harness_spawn_subagent` / `harness_complete_subagent` actually call it.
"""
...
def audit_db(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    # Also patch the path-resolver so HARNESS_ROOT doesn't sneak in.
...
# ── MCP tool: harness_query_subagent_events ────────────────────────────────
...
def test_query_tool_returns_empty_for_unknown_parent(
    ...
    raw = server.harness_query_subagent_events(
